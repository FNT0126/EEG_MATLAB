...
acc_RF           = mean(test_acc);
precision_RF   = TP/(TP+FP);
sensitivity_RF   = mean(test_sensitivity);
specificity_RF   = mean(test_specificity);
...
% 计算AUC
[XX, YY, ~, auc_RF] = perfcurve(test_labels, prob, 1);
disp(['Train_acc = ' num2str(train_acc_avg*100) '%' ...
    '（' num2str(sum(predict_label_1 == train_labels)) '/' ...
    num2str(length(train_labels)) '）' '  -->训练集'])
disp(['Test_acc = ' num2str(acc_RF*100) '%' ...
    '（' num2str(sum(predict_label_2 == test_labels)) '/' ...
    num2str(length(test_labels)) '）' '  -->测试集'])
disp(['Test_Sensitivity = ' num2str(sensitivity_RF*100) '%'])
disp(['Test_Specificity = ' num2str(specificity_RF*100) '%'])

save('test_data.mat','acc_RF','sensitivity_RF','specificity_RF','auc_RF','F1_score_RF', ...
    'precision_RF','-append')
...
% 显示图例和AUC值
lgd = legend('ROC Curve', ['AUC = ' num2str(auc_RF) ''], ...
...
%%  绘制误差曲线
figure
plot(1 : trees, err_avg,'LineWidth',1.5)
ax = gca;
ax.LineWidth = 1;
ax.FontName  = en;
ax.FontSize  = 12;
xlabel('决策树数目', 'fontsize', 14, 'FontName', zh)
ylabel('平均加权错误分类率', 'fontsize', 14, 'FontName', zh)
xlim([1, trees])
hold on
legend('平均加权错误分类率')
[err_min,minidx] = min(err_avg);
plot(minidx,err_min,'o','Color',[0.93,0.69,0.13],...
    'MarkerFaceColor', [0.85,0.33,0.10], 'MarkerSize', 4)
lgd = legend('平均误差曲线', '最小误差点', 'fontsize', 9, 'FontName', zh);
lgd.LineWidth = 0.75;
