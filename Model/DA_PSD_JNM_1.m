...
for times = 1:G
    ...
    for k = 1:K % K iterations
        ...
        [cv_class1,~,~] = classify(cv_train, cv_train, cv_TrainLabels, ...
            'diagLinear');
        [cv_class2,~,val_scores(:,:,k,times)] = classify(cv_validation, cv_train, cv_TrainLabels, ...
            'diagLinear');
...
auc_avg_LDA = mean(auc);
auc_std_LDA = std(auc);
acc_std_LDA = std(cv_acc);
cv_acc_Train_LDA  = mean(cv_acc_train);
cv_acc_LDA  = mean(cv_acc);
cv_sensitivity_LDA  = mean(cv_sensitivity);
cv_specificity_LDA  = mean(cv_specificity);
sensitivity_std_LDA = std(cv_sensitivity);
specificity_std_LDA = std(cv_specificity);
% temp(q) = cv_acc_LDA;
% end
% acc_avg = mean(temp);
save('cv_data.mat','cv_acc_LDA','cv_sensitivity_LDA','cv_specificity_LDA','cv_acc_Train_LDA', ...
    'acc_std_LDA','sensitivity_std_LDA','specificity_std_LDA','auc_avg_LDA','auc_std_LDA','-append')
...
acc_LDA   = mean(predict_label_2 == test_labels);
F1_score_LDA  = 2*(precision_LDA*sensitivity_LDA/(precision_LDA+sensitivity_LDA));

% 预测正类概率
prob = posterior(:,2);

% 计算AUC
[X, Y, ~, auc_LDA] = perfcurve(test_labels, prob, 1);
disp(['CV_Accuracy_Train = ' num2str(cv_acc_Train_LDA*100) '%'])
disp(['CV_Accuracy_Test = ' num2str(cv_acc_LDA*100) '%'])
disp(['Accuracy = ' num2str(train_acc*100) '%' ...
    '（' num2str(sum(predict_label_1 == train_labels)) '/' ...
    num2str(length(train_labels)) '）' '  -->训练集'])
disp(['Accuracy = ' num2str(acc_LDA*100) '%' ...
    '（' num2str(sum(predict_label_2 == test_labels)) '/' ...
    num2str(length(test_labels)) '）' '  -->测试集'])
disp(['Test_Sensitivity = ' num2str(sensitivity_LDA*100) '%'])
disp(['Test_Specificity = ' num2str(specificity_LDA*100) '%'])

save('test_data.mat','acc_LDA','sensitivity_LDA','specificity_LDA','auc_LDA','F1_score_LDA', ...
    'precision_LDA','-append')
...
% 显示图例和AUC值
lgd = legend('ROC Curve', ['AUC = ' num2str(auc_LDA) ''], 'Random Guessing', ...
