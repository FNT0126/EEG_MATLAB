...
%% 随机划分训练集与测试集
n            = size(all_psd_2,1); % 总样本数
m            = size(all_psd_2,2)-1; % 特征数
M            = m/7; % 分析脑部区域个数
tx_psd       = all_psd_2(1:n/2,:);
fsg_psd      = all_psd_2(n/2+1:end,:);
...
precision_SVM2   = tp/(tp+fp);
sensitivity_SVM2 = tp/(tp+fn); % compute specificity
specificity_SVM2 = tn/(tn+fp); % compute sensitivity
F1_score_SVM2  = 2*(precision_SVM2*sensitivity_SVM2/(precision_SVM2+sensitivity_SVM2));

% 预测正类概率
prob = prob_B(:,2);

% 计算AUC
[X, Y, ~, auc_SVM2] = perfcurve(test_labels, prob, 1);
...
disp(['Test_Sensitivity = ' num2str(sensitivity_SVM2*100) '%'])
disp(['Test_Specificity = ' num2str(specificity_SVM2*100) '%'])

save('test_data2.mat','acc_SVM2','sensitivity_SVM2','specificity_SVM2','auc_SVM2','F1_score_SVM2', ...
    'precision_SVM2','-append')
...
% 显示图例和AUC值
lgd = legend('ROC Curve', ['AUC = ' num2str(auc_SVM2) ''], 'Random Guessing', ...
